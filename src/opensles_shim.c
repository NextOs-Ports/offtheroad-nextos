...
static const int id_prefetch_tag = 9;
static const int id_metadata_tag = 10;
...
const SLInterfaceID sl_IID_PREFETCHSTATUS = &id_prefetch_tag;
...
static int g_audio_log = -1;
static int audio_log_on(void) {
  if (g_audio_log < 0) g_audio_log = getenv("HW_AUDIOLOG") ? 1 : 0;
  return g_audio_log;
}
...
static AudioPlayer *alloc_player(void) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (!g_players[i].active) {
      ...
      if (audio_log_on()) {
        int n = 0;
        for (int k = 0; k < MAX_PLAYERS; k++) if (g_players[k].active) n++;
        debugPrintf("[audio] player %d ALLOC (ativos=%d)\n", i, n);
      }
...
static SLresult play_SetPlayState(void *self, SLuint32 state) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].play_ptr == itf_ptr) {
      AudioPlayer *p = &g_players[i];
      if (audio_log_on())
        debugPrintf("[audio] player %d SetPlayState(%u -> %u)\n", i, p->play_state, state);
...
static void player_Destroy(void *self) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (&g_players[i].obj_ptr == obj_ptr) {
      AudioPlayer *p = &g_players[i];
      if (audio_log_on()) debugPrintf("[audio] player %d DESTROY\n", i);
      if (g_audio_dev) SDL_LockAudioDevice(g_audio_dev);
      p->play_state = SL_PLAYSTATE_STOPPED;
      p->active = 0;
      if (g_audio_dev) SDL_UnlockAudioDevice(g_audio_dev);
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    ...
    if (p->decoder_done && !p->headatend_fired) {
      if (ring_readable(p) == 0) {
        p->headatend_fired = 1;
        if (audio_log_on()) debugPrintf("[audio] player %d HEADATEND\n", i);
