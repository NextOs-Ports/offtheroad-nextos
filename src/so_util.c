...
/* Igual ao hook_arm64, mas devolve um trampolim que executa a funcao ORIGINAL
 * (primeira instrucao salva + salto para addr+4). So vale para funcoes cuja
 * primeira instrucao seja independente de PC (sub sp / stp) -- sempre conferir. */
uintptr_t hook_arm64_trace(uintptr_t addr, uintptr_t dst) {
  if (addr == 0)
    return 0;
  if (g_tramp_pool == NULL) {
    g_tramp_pool = (uint8_t *)(((uintptr_t)data_base + data_size + 15) & ~(uintptr_t)15);
  }
  uint32_t *tramp = (uint32_t *)(g_tramp_pool + (size_t)g_tramp_used * 16);
  g_tramp_used += 2; /* 32 bytes: instrucao original + salto de 64 bits */
  uint32_t orig = *(uint32_t *)addr;
  tramp[0] = orig;
  tramp[1] = 0xd503201fu;         // NOP (alinha o literal em 8)
  tramp[2] = 0x58000051u;         // LDR X17, #0x8
  tramp[3] = 0xd61f0220u;         // BR X17
  *(uint64_t *)(tramp + 4) = (uint64_t)(addr + 4);
...
/* Troca o slot PLT/GOT de um simbolo EXPORTADO da propria lib por um wrapper
 * nosso. so_relocate liga JUMP_SLOT de simbolo definido a propria definicao;
 * este helper re-liga por NOME depois do load. Toda chamada interna que passa
 * pelo PLT (caso de setupLevelOfDetail: todos os call sites usam @plt) passa a
 * cair no wrapper; o endereco real continua acessivel por so_find_addr. O GOT
 * fica RWX (ver so_finalize), entao nao ha mprotect aqui. */
int so_interpose_export(const char *name, uintptr_t wrapper) {
  int patched = 0;
  for (int i = 0; i < elf_hdr->e_shnum; i++) {
    char *sh_name = shstrtab + sec_hdr[i].sh_name;
    if (strcmp(sh_name, ".rela.plt") != 0)
      continue;
    Elf64_Rela *rels = (Elf64_Rela *)((uintptr_t)load_base + sec_hdr[i].sh_addr);
    for (int j = 0; j < (int)(sec_hdr[i].sh_size / sizeof(Elf64_Rela)); j++) {
      if (ELF64_R_TYPE(rels[j].r_info) != R_AARCH64_JUMP_SLOT)
        continue;
      ...
      if (strcmp(dynstrtab + sym->st_name, name) != 0)
        continue;
      uintptr_t *slot = (uintptr_t *)((uintptr_t)load_base + rels[j].r_offset);
      *slot = wrapper;
      patched++;
    }
  }
